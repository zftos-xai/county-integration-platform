...
| 接入配置 | 机构与权限 | 维护机构代码、接口范围和访问权限 | 可见 | 不显示 |
...
- **解决的问题**：哪些机构和系统可以接入平台，每个机构可以使用哪些接口和数据范围。
- **主要对象**：机构、平台机构代码、外部机构代码、接入系统、接口范围和状态。
- **主要内容**：机构基本信息、代码对应、允许接口、访问范围和配置变更记录。
- **主要动作**：新增或维护机构、配置接口范围、启用或停用接入、查看变更记录。
- **操作边界**：前端配置结果必须由服务端执行身份、机构范围和接口权限校验；停用机构不能继续创建新请求。
- **页面说明**：`维护接入机构、编码对应和接口授权范围。`
...
| 基础配置 | 接入配置 | 启用 | 使用明确的接入对象作为子菜单 |
| 系统管理 | 系统管理 | 分阶段启用 | 身份权限和真实参数具备后逐项开放 |

#### 分阶段扩展顺序

1. **当前原型**：运行总览、基础数据、业务接口、交换记录、机构与权限、告警管理、审计记录。
2. **运行接入后**：新增接口运行和发送任务，接入真实运行指标与控制状态。
3. **任务闭环形成后**：新增我的工作台，支持领取、分配、期限和持续观察。
4. **权限体系完成后**：扩展用户与角色、操作权限。
5. **正式规则批准后**：扩展编码对应、数据保存规则和经批准的系统参数。
...
| 机构与权限 | 标准列表页 | 机构代码、接入系统、接口范围、状态 | 详情抽屉 |
