...
| --- | --- | --- | --- | --- |
| 运行监控 | 运行总览 | 查看数据传递状态、未解决事项和待处理入口 | 可见 | 可见 |
| 基础数据处理 | 基础数据 | 查看同步结果、平台保存范围和HIS读取状态 | 可见 | 可见 |
| 业务接口处理 | 业务接口 | 维护接口关系、规则版本以及当前是否允许新请求 | 可见 | 不显示 |
| 业务接口处理 | 交换记录 | 追踪受理、发送、目标系统返回结果、业务结果和保存状态 | 可见 | 可见 |
| 运行处理 | 告警管理 | 处理同步、接口、结果未知和数据到期问题 | 可见 | 可见 |
| 运行处理 | 审计记录 | 追溯配置、核查和处理动作 | 可见 | 可见 |
...
系统能够自动完成的检测、暂停、恢复、清理和重试由系统执行。人工操作负责确认判断、登记处理依据、确定责任、批准受控动作和处理系统无法自动判断的情况。
...
- **主要内容**：接口方向、处理方式、数据保存策略、当前版本、是否允许新请求和关联交换记录。
- **主要动作**：查看接口定义、维护规则版本、调整启停状态、核对关联交换。
- **操作边界**：修改当前接口定义不覆盖历史交换记录保存的规则快照；不允许进入的接口不得生成新交换记录。
- **页面说明**：`维护接口规则、版本以及当前是否允许新请求。`

#### 交换记录

- **解决的问题**：某笔业务请求从受理到发送、目标系统返回结果和后续核查进行到了哪一步。
- **主要对象**：业务单号、请求编号、机构、接口、受理时规则版本、发送任务、目标系统返回结果和处理记录。
- **主要内容**：业务结果、系统处理进度、数据保存状态、可供核查的信息和当前责任。
- **主要动作**：查看详情、领取核查、登记核查结果、安排下次检查、进入关联接口或告警。
- **操作边界**：结果未知时不能直接重发；不能用来源最新版本替换原请求版本；人工登记不能伪造目标系统成功。
- **页面说明**：`按业务单号追踪受理、发送、目标系统返回结果和处理进度。`
...
- **主要动作**：领取告警、限制新请求、执行受控探测或重试、登记处理依据、转入持续观察、完成处理。
...
- **主要内容**：时间、操作人、动作、对象、变更摘要、依据和关联编号。
...
| 基础数据 | 可选 | 当前同步失败或接口资料不完整的数据类别数 | 需要关注的数据 |
...
参考常见管理平台使用“工作台、数据总览、告警管理、接口监控、数据交换、基础配置、系统管理”的方式时，必须先明确每个菜单管理什么以及用户要完成哪些操作。只有同时具备独立管理对象、独立列表、独立权限，并且支持读取、修改、保存后确认结果时，才建立一级页面；仅是筛选条件或详情页签的内容，不单独建立菜单。
...
**业务对象**：接口服务地址、连接状态、调用成功率、响应时间、连续失败次数和运行控制状态。
...
**交换记录业务对象**：一次业务请求从受理、生成发送任务、发送、接收目标系统返回结果到核查结果的全过程记录。
...
| 业务接口 | 接口定义、系统关系、交易码和版本 | 维护调用方向、规则、是否允许新请求和处理方式 |
...
3. **任务已支持领取、分配、处理和确认结果后**：新增我的工作台，显示负责人、完成期限和后续检查事项。
4. **接口联调后**：将用户、角色、机构和字典的合成数据切换为真实服务端数据。
5. **正式参数批准后**：在代码中注册参数定义，再开放对应的参数值维护入口。

任何扩展菜单上线前都必须通过本规范第12章定义的VISTA验收，确认菜单职责、角色权限、页面结构、数据来源和完整业务处理过程均已成立。
...
```text
业务接口处理
交换记录    按业务单号追踪受理、发送、目标系统返回结果和处理进度
```
...
| 业务接口 | 标准列表页 | 系统关系、交易码、规则版本、是否允许新请求、保存策略 | 详情抽屉 |
| 交换记录 | 主从处理工作区 | 业务记录、接口快照、发送目标系统返回结果、业务结果、数据保存 | 右侧处理区 |
...
| 审计记录 | 标准只读列表 | 时间、操作者、动作、对象、变更摘要 | 可选只读详情 |
...
禁止使用大面积渐变、装饰性插画、过度圆角、悬浮玻璃效果和无法解释的数据图表。界面以运行状态、业务对象、核查依据和可执行操作为重点。
...
- 人工操作只用于登记判断依据、明确责任和执行获准操作，不代替系统自动处理；
- 操作列在最低桌面宽度下始终可见；
- 所有列表具有标准分页；
...
证据必须绑定目标页面地址、角色、视口、代码版本和采集时间。截图、脚本退出码或报告结构有效都不能单独提升验收结果。
...
本地界面通过只代表当前代码版本和桌面环境符合UI验收要求。真实API、SQL Server、服务端权限、生产数据和正式发布必须在各自环境另行验证。
...
```text
.codex/evidence/vista-<日期>-<页面地址简称>/
```

至少包含：

- `contract.json`：VISTA 工具固定使用的文件名，内容是验收检查说明；
...
验收检查说明和报告使用VISTA校验脚本检查结构。结构校验通过只说明记录完整，不代表页面自动通过。
...
接口详情必须把配置拆成业务定义、连接与调用、执行控制、版本生效四组。除提供方、调用方、源目标系统和交易码外，还需明确服务地址或服务标识、传输方式、超时、重试规则、重复识别规则、成功判定、生效时间和审批状态。配置页不展示密码、令牌等敏感凭据。

### 13.2 机构管理

当前机构管理只维护服务端已有模型支撑的机构主数据、层级、有效期和外部代码对应。机构接口授权在服务端数据模型、接口规范和权限校验规则批准前不提供可编辑入口；后续接入时必须按“机构 + 接口”登记调用方向、服务身份、数据范围、有效期和批准依据，并由服务端逐次校验。
...
- 基础数据同步失败需显示失败批次、字段、校验规则、已删除或遮盖敏感字段的样例、来源责任方、重试范围、执行次数和错误代码；受控重试必须再次确认范围。
- 数据到期清理失败需显示发送控制、清理范围、执行结果、执行次数、最近错误和到期时间；清理结果不得改写业务结果。
- 审计列表必须提供只读详情入口，展示操作人、时间、业务对象、动作、处理摘要、关联标识和可解释的变更前后状态。
...
| 状态类别 | 页面标准名称 | 使用范围 |
| --- | --- | --- |
| 业务结果 | 尚未发送、结果未知、明确未写入、已确认写入、校验未通过、查询失败、越权拒绝、重复已识别 | 交换记录、告警影响范围、发送记录和版本对比 |
...
| 接口运行 | 接口资料不完整、待联调、已启用（演示）、已启用、已暂停、已停用 | 业务接口列表、配置详情和运行控制 |
| 基础数据同步 | 同步正常、同步失败、接口资料不完整 | 基础数据列表和详情 |
| HIS读取 | 未开放、待开放、可读取 | 基础数据列表和HIS读取详情 |
| 通用启停 | 已启用、已停用 | 机构、权限和编码对应 |

统一规则如下：

- “成功”不能单独作为交换业务结果；取得目标系统返回结果，或按相同业务编号查询到对应记录后，才显示“已确认写入”。
...
- 人工入口必须说明人工介入原因、责任角色、允许动作、禁止动作和完成所需记录；系统能够按既定规则完成的处理不生成通用人工办理入口。
