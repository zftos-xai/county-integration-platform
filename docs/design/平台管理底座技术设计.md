...
| 模块 | 职责 | 唯一写入对象 | 禁止事项 |
| --- | --- | --- | --- |
| `modules/organization` | 平台机构档案、层级和启停 | `org_organization` | 不写用户、角色或上游基础数据 |
| `system/identity` | 本地身份、初始化、会话与改密边界 | `sys_user`及会话对象 | 不将外部业务鉴权当作管理身份 |
| `system/access` | 角色、权限和用户机构范围 | `sys_role`、`sys_permission`及关联表 | 不直接写机构表 |
| `system/configuration` | 参数、字典、外部系统和环境端点 | `sys_parameter_value`、`sys_dictionary_*`、`sys_external_*` | 不保存医疗业务目录或明文凭证 |
| `modules/audit` | 管理操作和安全事件追溯 | `audit_management_event` | 不提供业务修改、删除或通用报文保存 |

模块间只通过应用层边界或不可变读模型协作，不得直接调用其他模块的Mapper。
...
| 版本 | 业务域 | 对象 | 处理决定 |
| --- | --- | --- | --- |
| V0100 | `exchange` | 现有交换记录 | 保留已提交版本，避免改写可能已执行的历史；功能扩展继续冻结 |
| V0200 | `organization` | `org_organization` | 先建立被用户、权限范围和配置引用的机构主数据 |
| V0300 | `system` | 用户、角色、权限、机构范围、参数、字典、外部系统与端点 | 通过外键引用V0200机构 |
| V0400 | `audit` | `audit_management_event` | 引用用户/机构只作查询关联，同时保留操作人快照 |

Flyway版本是历史执行顺序，不代表功能开发优先级。V0100已进入Git历史，在无法证明从未被共享环境执行的情况下不重命名或改写。
