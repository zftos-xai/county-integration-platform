...
test('医疗目录在列表展示地区和HIS启用值，仅以固定选项筛选启用原值', async () => {
  const page = await readFile(pagePath, 'utf8')
  assert.match(page, /<th scope="col">地区<\/th>/)
  assert.match(page, /<th scope="col">HIS 启用值<\/th>/)
  assert.match(page, /<td :title="item\.region \|\| 'HIS 未返回地区'">/)
...
test('100-004 可选字段经接口校验后可在展开详情逐项核对', async () => {
  ...
  for (const field of ['dosageForm', 'packageUnit', 'conversionFactor', 'approvalNumber', 'standardCode']) {
...
test('列表不重复展示助记码，双击数据行可切换详情且不劫持按钮或链接', async () => {
  const page = await readFile(pagePath, 'utf8')
  assert.doesNotMatch(page, /<small v-if="item\.mnemonicCode">/)
