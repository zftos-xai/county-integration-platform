...
const foundationStorageKey = 'county-integration-foundation-v2'
...
const history = ref<SyncEvent[]>([
  { id: 'SIM-SYNC-0915-006', dataset: '基层科室', startedAt: '2026-09-15 08:06', endedAt: '2026-09-15 08:06', mode: '增量（演示）', received: '12', changed: '3', failed: '0', result: '成功', evidence: 'SIM-RUN-006' },
  { id: 'SIM-SYNC-0915-005', dataset: '基层机构', startedAt: '2026-09-15 08:05', endedAt: '2026-09-15 08:05', mode: '增量（演示）', received: '4', changed: '1', failed: '0', result: '成功', evidence: 'SIM-RUN-005' },
  { id: 'SIM-SYNC-0915-004', dataset: '药品目录', startedAt: '2026-09-15 10:31', endedAt: '2026-09-15 10:31', mode: '增量（演示）', received: '200', changed: '0', failed: '200', result: '失败', evidence: 'SIM-VALIDATION-004' },
  { id: 'SIM-SYNC-0915-003', dataset: '公共字典', startedAt: '2026-09-15 02:03', endedAt: '2026-09-15 02:03', mode: '全量（演示）', received: '48', changed: '0', failed: '0', result: '成功', evidence: 'SIM-RUN-003' },
])
...
const canRetry = computed(() => props.role === 'operator' && selectedDataset.value?.status === '同步失败' && retryEvidence.value.trim() && retryNote.value.trim())
...
function resetAction() { retryEvidence.value = ''; retryNote.value = ''; operationMessage.value = '' }
...
function retrySync() {
  ...
  history.value.unshift({ id: 'SIM-SYNC-0915-007', dataset: selectedDataset.value.name, startedAt: '2026-09-15 12:08', endedAt: '2026-09-15 12:08', mode: '受控重试（演示）', received: '200', changed: '18', failed: '0', result: '重试成功', evidence: retryEvidence.value.trim() })
...
<template>
  <section class="foundation-page">
    ...
    <div class="data-filterbar foundation-filter" role="search" aria-label="基础数据筛选"><label class="data-filter-search"><Search :size="18" /><input v-model="query" :aria-label="tab === 'history' ? '搜索同步记录' : '搜索基础数据'" :placeholder="tab === 'history' ? '搜索同步编号、数据类别或依据' : '搜索数据类别、来源或编号'" /></label><label v-if="tab !== 'history'"><span>状态</span><select v-model="status" aria-label="状态"><option>全部状态</option><option>正常</option><option>接入条件待确认</option><option>同步失败</option></select></label><span class="data-filter-count">共 {{ activeRows.length }} 条</span></div>

    <div class="card data-receipt-table foundation-table"><div class="table-responsive">
      <table v-if="tab === 'datasets'" class="table table-vcenter"><thead><tr><th>基础数据</th><th>来源与取得方式</th><th>同步计划</th><th>最近结果</th><th>平台保存</th><th>HIS读取</th><th>状态</th><th>操作</th></tr></thead><tbody><tr v-for="item in pagedDatasetRows" :key="item.id"><td><button class="data-record-link" @click="openDataset(item)">{{ item.name }}</button><small>{{ item.id }}</small></td><td><strong>{{ item.source }}</strong><small>{{ item.method }}</small></td><td>{{ item.updateRule }}</td><td><strong>{{ item.syncedAt }}</strong><small>{{ item.lastResult }}</small></td><td><strong>{{ item.count === '—' ? '未同步' : `${item.count} 条` }}</strong><small>{{ item.count === '—' ? '尚未形成平台数据' : '保存批准字段' }}</small></td><td><button class="foundation-access-link" @click="openDataset(item, 'access')"><strong>{{ datasetAccessState(item.name) }}</strong><small>{{ datasetAccessDetail(item.name) }}</small></button></td><td><span class="prototype-tag" :class="stateClass(item.status)">{{ item.status }}</span></td><td><button class="btn btn-outline-primary btn-sm" @click="openDataset(item)">{{ item.status === '同步失败' ? '处理' : '详情' }}</button></td></tr></tbody></table>
      <table v-else class="table table-vcenter"><thead><tr><th>同步编号</th><th>基础数据</th><th>开始 / 结束时间</th><th>同步方式</th><th>收到</th><th>变更</th><th>失败</th><th>结果 / 依据</th><th>操作</th></tr></thead><tbody><tr v-for="item in pagedHistoryRows" :key="item.id"><td>{{ item.id }}</td><td>{{ item.dataset }}</td><td>{{ item.startedAt }}<small>{{ item.endedAt }}</small></td><td>{{ item.mode }}</td><td>{{ item.received }}</td><td>{{ item.changed }}</td><td>{{ item.failed }}</td><td><span class="prototype-tag" :class="stateClass(item.result)">{{ item.result }}</span><small>{{ item.evidence }}</small></td><td><button class="btn btn-link btn-sm" @click="openDatasetFromHistory(item)">查看对应数据</button></td></tr></tbody></table>
    ...
    <div v-if="selectedDataset" class="data-record-overlay" @click.self="closeDetail">
      <aside class="data-record-drawer foundation-drawer" role="dialog" aria-modal="true" aria-label="基础数据详情">
        ...
        <div class="drawer-summary foundation-detail-summary"><div><span>同步状态</span><strong><em class="status-pill" :class="stateClass(selectedDataset.status)">{{ selectedDataset.status }}</em></strong></div><div><span>最近同步</span><strong>{{ selectedDataset.syncedAt }}</strong></div><div><span>平台数据量</span><strong>{{ selectedDataset.count }}</strong></div><div><span>HIS读取</span><strong>{{ datasetAccessState(selectedDataset.name) }}</strong></div></div>
