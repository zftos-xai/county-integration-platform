  ...
  <section class="directory-result-panel">
    ...
    <div v-else class="directory-result-grid">
      <article v-for="result in props.results" :key="result.directoryType" class="directory-result-card" :class="result.status.toLowerCase()">
        ...
        <div class="batch-detail-grid-scroll result-card-scroll" role="region" :aria-label="`${typeLabels[result.directoryType]}同步统计`" tabindex="0">
          <dl class="batch-detail-grid result-card-fields">
            ...
            <div v-if="result.invalidCount + result.conflictCount > 0" class="batch-detail-grid-wide"><dt>校验问题</dt><dd>{{ result.invalidCount + result.conflictCount }} 条</dd></div>
          </dl>
        </div>
...
<style scoped>
.directory-result-panel { border: 1px solid #dce5e7; border-radius: 6px; background: #fff; }
.directory-result-panel > header { padding: 10px 16px; border-bottom: 1px solid #e9eef0; }
.directory-result-panel h3 { margin: 0; color: #2d424b; font-size: 14px; }
.directory-result-panel p { margin: 5px 0 0; color: #718189; font-size: 12px; line-height: 1.55; }
...
.directory-result-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 9px; padding: 9px; }
.directory-result-card { min-width: 0; padding: 10px; border: 1px solid #dbe7e6; border-radius: 5px; background: #fbfefd; }
.directory-result-card.failed, .directory-result-card.result_unknown { border-color: #ecd2c9; background: #fffaf8; }
.directory-result-card > header { display: flex; align-items: center; justify-content: space-between; gap: 8px; }
.directory-result-card > header strong { color: #29424b; font-size: 14px; }
.directory-result-card > header span { display: inline-flex; align-items: center; gap: 4px; color: #237461; font-size: 12px; white-space: nowrap; }
.directory-result-card > header span.failed, .directory-result-card > header span.result_unknown { color: #ae5039; }
.result-card-scroll { margin-top: 9px; }
.result-card-fields { width: 100%; grid-template-columns: repeat(2, minmax(0, 1fr)); }
.result-card-fields > div { grid-template-columns: 78px minmax(max-content, 1fr); }
.result-card-fields > .batch-detail-grid-wide { grid-column: 1 / -1; }
.result-failure { padding-top: 9px; border-top: 1px solid #f0d7d1; color: #7f5146 !important; }
.spinning { animation: spin 1s linear infinite; }
@keyframes spin { to { transform: rotate(360deg); } }
@media (max-width: 900px) { .directory-result-grid { grid-template-columns: 1fr; } }
</style>
