  ...
  <section class="medical-result-panel">
    ...
    <div v-else class="result-grid">
      <article v-for="result in props.results" :key="result.directoryType" class="result-card" :class="result.status.toLowerCase()">
        ...
        <dl><div><dt>声明 / 返回</dt><dd>{{ result.declaredCount ?? '—' }} / {{ result.returnedCount }}</dd></div><div><dt>新增 / 更新</dt><dd>{{ result.createdCount }} / {{ result.updatedCount }}</dd></div><div><dt>当前有效</dt><dd>{{ result.activeCount === null ? '未更新' : `${result.activeCount} 条` }}</dd></div><div><dt>无效 / 重复 / 冲突</dt><dd>{{ result.invalidCount }} / {{ result.duplicateCount }} / {{ result.conflictCount }}</dd></div></dl>
...
<style scoped>
.result-failure { overflow-wrap: anywhere; }
