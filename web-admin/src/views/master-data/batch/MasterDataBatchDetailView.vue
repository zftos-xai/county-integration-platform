<!-- 同步记录详情页面：只展示100-003执行事实和当前有效数据结果，不提供人工处理入口。 -->
<script setup lang="ts">
import { AlertCircle, CheckCircle2, RefreshCw } from 'lucide-vue-next'
import { onMounted, ref } from 'vue'
import { useRoute } from 'vue-router'
import { getHospitalDirectorySyncResults, getMasterDataBatch } from '@/api/master-data/batch'
import type { HospitalDirectorySyncResult, MasterDataBatchSummary } from '@/api/master-data/batch'
...
async function load() {
  ...
  try {
    const [summary, results] = await Promise.all([
      getMasterDataBatch(batchId),
      getHospitalDirectorySyncResults(batchId),
    ])
    batch.value = summary
...
function statusLabel(current: MasterDataBatchSummary) {
  if (current.status === 'FAILED' && current.failureCode === 'HIS_BUSINESS_FAILURE') return '等待接口授权'
  return ({ CREATED: '尚未开始', FETCHING: '正在从 HIS 取得', COMPLETED: '已完成对账',
    COMPLETED_WITH_ERRORS: '部分未完成', COMPLETED_WITH_UNKNOWN: '部分结果未知',
    FAILED: '未完成', RESULT_UNKNOWN: '结果待确认' } satisfies Record<MasterDataBatchSummary['status'], string>)[current.status]
}
...
function failureTitle(current: MasterDataBatchSummary) {
  if (current.failureCode === 'HIS_BUSINESS_FAILURE') return 'HIS 未授予目录数据权限'
...
function outcomeSummary(current: MasterDataBatchSummary) {
  ...
  if (current.status === 'COMPLETED_WITH_UNKNOWN') return '部分目录查询结果无法确认；已完成目录类型已更新，结果未知类型没有改动。'
  if (current.status === 'FAILED' && current.failureCode === 'HIS_BUSINESS_FAILURE') return '读取医院综合目录时被 HIS 拒绝；平台没有取得可确认的数据。'
...
function nextStep(current: MasterDataBatchSummary) {
  if (current.failureCode === 'HIS_BUSINESS_FAILURE') return '请接口提供方开通该机构的 100-003 目录查询权限；确认后从同步列表新建一次同步。'
...
<template>
  <section class="sync-record-page">
    <header class="record-heading">
      <div><small>医院综合目录 / 同步记录</small><h2>同步结果</h2><p v-if="batch">{{ batch.organizationName ?? batch.organizationCode }} · {{ batch.environment === 'PRODUCTION' ? '生产环境' : batch.environment === 'TEST' ? '测试环境' : '开发环境' }} · 批次号 {{ batch.batchNo }}</p><p v-else>一次自动取得科室、医生、病区和床位；每种目录完整通过自动校验后直接更新当前数据。</p></div>
      <button class="work-quiet-button" type="button" :disabled="loading" @click="load"><RefreshCw :size="16" />刷新</button>
    </header>
    ...
    <template v-else-if="batch">
      ...
      <section class="record-card">
        <h3>本次调用</h3>
        <dl class="record-flow"><div><dt>同步机构</dt><dd>{{ batch.organizationName ?? batch.organizationCode }}</dd></div><div><dt>数据范围</dt><dd>科室、医生、病区、床位</dd></div><div><dt>HIS交易</dt><dd>{{ batch.dataTradeCode }}</dd></div><div><dt>结果</dt><dd>{{ statusLabel(batch) }}</dd></div></dl>
      </section>
      <section v-if="batch.counts.returned > 0 || batch.status === 'COMPLETED'" class="record-card">
        ...
        <p v-if="batch.status === 'COMPLETED'" class="record-directory-link"><RouterLink to="/master-data/directory">查看当前数据目录</RouterLink></p>
      </section>
