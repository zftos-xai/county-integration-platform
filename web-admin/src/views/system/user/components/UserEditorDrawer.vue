...
type EditorSection = 'base' | 'access' | 'password'
...
<template>
  <div class="drawer-layer" @mousedown.self="emit('close')">
    <section ref="dialogRef" class="user-drawer" role="dialog" aria-modal="true" aria-labelledby="user-editor-title" tabindex="-1" @keydown="handleDialogKeydown">
      <header><div><span>{{ mode === 'create' ? '新增平台用户' : mode === 'edit' ? '编辑用户' : '用户详情' }}</span><h2 id="user-editor-title">{{ mode === 'create' ? '建立用户账号' : selected?.displayName }}</h2><small v-if="selected">登录名：{{ selected.loginName }}</small></div><button class="prototype-icon" type="button" aria-label="关闭" @click="emit('close')"><X :size="18" /></button></header>
      <div v-if="isDetailLoading" class="drawer-loading"><LoaderCircle class="spinning" :size="22" />正在读取最新版本…</div>
      <nav v-if="selected" class="user-editor-tabs" aria-label="用户编辑区域">
      ...
      <div class="user-drawer-body">
        ...
        <form v-if="activeSection === 'base'" class="user-form" @submit.prevent="emit('submitBase')">
          <section><h3>基础资料</h3><p>登录名用于登录，用户姓名用于页面和审计展示；主要机构属于当前账号的数据范围。</p></section>
        ...
        <template v-if="selected && activeSection === 'access'">
          <section class="access-section"><header><div><h3>功能角色</h3><p>角色决定可使用的管理功能；修改当前账号角色后需重新登录。</p></div><button v-if="canManageAccess" class="work-quiet-button" type="button" :disabled="isSaving || (!addedRoleNames.length && !removedRoleNames.length)" @click="emit('saveRoles')">保存角色</button></header><div v-if="addedRoleNames.length || removedRoleNames.length" class="change-summary" role="status"><strong>待保存的角色变化</strong><span v-if="addedRoleNames.length">新增：{{ addedRoleNames.join('、') }}</span><span v-if="removedRoleNames.length">取消：{{ removedRoleNames.join('、') }}</span></div><div v-if="roles.length" class="choice-grid"><label v-for="role in enabledRoles" :key="role.id"><input v-model="roleDraft" type="checkbox" :value="role.id" :disabled="!canManageAccess" /><span><strong>{{ role.roleName }}</strong><small>{{ role.roleCode }}</small></span></label></div><p v-else class="section-empty">当前账号无权读取角色清单。</p></section>
          <section class="access-section"><header><div><h3>数据范围</h3><p>决定可查看哪些机构的数据；必须包含主要机构，且不能超出当前账号可管理范围。</p></div><button v-if="canManageAccess" class="work-quiet-button" type="button" :disabled="isSaving || (!addedScopeNames.length && !removedScopeNames.length)" @click="emit('saveScopes')">保存数据范围</button></header><div v-if="addedScopeNames.length || removedScopeNames.length" class="change-summary" role="status"><strong>待保存的机构范围变化</strong><span v-if="addedScopeNames.length">新增：{{ addedScopeNames.join('、') }}</span><span v-if="removedScopeNames.length">取消：{{ removedScopeNames.join('、') }}</span></div><div v-if="organizations.length" class="choice-grid"><label v-for="organization in enabledOrganizations" :key="organization.id"><input v-model="scopeDraft" type="checkbox" :value="organization.id" :disabled="!canManageAccess || organization.id === selected.primaryOrganizationId" /><span><strong>{{ organization.organizationName }}</strong><small>{{ organization.organizationCode }}{{ organization.id === selected.primaryOrganizationId ? ' · 主要机构' : '' }}</small></span></label></div><p v-else class="section-empty">当前账号无权读取机构清单。</p></section>
        </template>
        <template v-if="selected && canWrite && activeSection === 'password'">
          <section class="access-section password-operation"><header><div><h3>重置临时密码</h3><p>重置后当前登录状态失效，用户下次登录必须修改密码。</p></div><button v-if="!isPasswordResetOpen" class="work-quiet-button" type="button" @click="isPasswordResetOpen = true"><KeyRound :size="15" />重置密码</button></header><form v-if="isPasswordResetOpen" class="password-reset" @submit.prevent="emit('submitPasswordReset')"><label class="visually-hidden" for="temporary-password">一次性临时密码</label><input id="temporary-password" v-model="temporaryPassword" type="password" maxlength="128" autocomplete="new-password" placeholder="输入一次性临时密码" /><button class="work-quiet-button" type="button" @click="isPasswordResetOpen = false; temporaryPassword = ''">取消</button><button class="prototype-button" type="submit" :disabled="isSaving">确认重置</button></form></section>
        </template>
      </div>
    </section>
  </div>
...
.user-drawer > header small { display: block; margin-top: 3px; color: #75828c; font-size: 11px; }
.drawer-loading { padding: 9px 20px; background: #edf5f2; color: #456c61; display: flex; align-items: center; gap: 8px; font-size: 12px; }
.user-editor-tabs { min-height: 45px; padding: 0 20px; border-bottom: 1px solid #e2e7e9; display: flex; gap: 22px; background: #fbfcfc; }
.user-editor-tabs button { min-height: 45px; padding: 0; border: 0; border-bottom: 2px solid transparent; background: transparent; color: #6d7b86; font-size: 12px; font-weight: 600; }
.user-editor-tabs button.active { border-bottom-color: #147467; color: #12695d; }
.user-editor-tabs button:focus-visible { outline: 2px solid #1a8173; outline-offset: -4px; }
...
.access-section { padding: 16px 0; border-top: 1px solid #e2e7e9; }
.user-drawer-body > .access-section:first-child { padding-top: 0; border-top: 0; }
...
.password-operation { max-width: 100%; }
