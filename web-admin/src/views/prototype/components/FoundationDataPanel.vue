...
type DataStatus = '正常' | '接口资料不完整' | '同步失败'
type Dataset = {
...
type DataView = {
  ...
  scope: string; version: string; verifiedAt: string; status: '已发布（演示）' | '待发布' | '接口资料不完整'
}
type SyncEvent = {
...
const datasets = ref<Dataset[]>([
  { id: 'SIM-MD-ORG', name: '基层机构', source: '基层系统', method: '接口', updateRule: '初始全量 + 每日增量', count: '24', sourceUpdatedAt: '2026-09-15 08:00', syncedAt: '2026-09-15 08:05', view: 'SIM_VIEW_ORG', status: '正常', stableKey: '机构唯一标识（正式字段待确认）', versionField: '源更新时间（正式含义待确认）', stopRule: '停用后保留标识并标记无效（待确认）', retainedFields: '机构标识、名称、行政区划、有效状态（演示）', historyVisibility: 'HIS默认仅查询有效数据（待确认）', lastResult: '收到4条，变更1条，失败0条', failureReason: '—' },
  ...
  { id: 'SIM-MD-STAFF', name: '基层人员', source: '基层系统', method: '未确认', updateRule: '主键和停用含义待确认', count: '—', sourceUpdatedAt: '—', syncedAt: '—', view: '未发布', status: '接口资料不完整', stableKey: '待确认', versionField: '待确认', stopRule: '离岗、停用和跨机构变更含义待确认', retainedFields: '不得根据HIS需求推测字段', historyVisibility: '待确认', lastResult: '未执行', failureReason: '缺少正式来源字段、主键和停用规则' },
  { id: 'SIM-MD-ITEM', name: '诊疗项目目录', source: '基层系统', method: '未确认', updateRule: '编码版本待确认', count: '—', sourceUpdatedAt: '—', syncedAt: '—', view: '未发布', status: '接口资料不完整', stableKey: '项目编码及机构范围待确认', versionField: '目录版本待确认', stopRule: '项目停用与替代关系待确认', retainedFields: '不得根据HIS需求推测字段', historyVisibility: '待确认', lastResult: '未执行', failureReason: '缺少正式来源规则和目录版本含义' },
...
const views: DataView[] = [
  ...
  { id: 'SIM-VIEW-STAFF', name: '视图名称待确认', dataset: '基层人员', consumer: '县医院HIS', account: '待授权', scope: '用途、字段和历史范围待确认', version: '—', verifiedAt: '—', status: '接口资料不完整' },
]
...
function datasetAccessState(dataset: string) {
  ...
  if (!view || view.status === '接口资料不完整') return '未开放'
  return view.status === '待发布' ? '待开放' : '可读取'
}
function datasetAccessDetail(dataset: string) {
  const view = views.find(item => item.dataset === dataset)
  if (!view || view.status === '接口资料不完整') return '尚未开放读取'
...
<template>
  <section class="foundation-page">
    <div class="foundation-commandbar">
      ...
      <dl><div><dt>数据类别</dt><dd>{{ datasets.length }}</dd></div><div><dt>同步失败</dt><dd class="danger">{{ datasets.filter(item => item.status === '同步失败').length }}</dd></div><div><dt>条件待确认</dt><dd>{{ datasets.filter(item => item.status === '接口资料不完整').length }}</dd></div><div><dt>HIS可读取</dt><dd>{{ views.filter(item => item.status === '已发布（演示）').length }}</dd></div></dl>
    ...
    <div class="data-filterbar foundation-filter" role="search" aria-label="基础数据筛选"><label class="data-filter-search"><Search :size="18" /><input v-model="query" :aria-label="tab === 'history' ? '搜索同步记录' : '搜索基础数据'" :placeholder="tab === 'history' ? '搜索同步编号、数据类别或依据' : '搜索数据类别、来源或编号'" /></label><label v-if="tab !== 'history'"><span>同步状态</span><select v-model="status" aria-label="同步状态"><option>全部状态</option><option value="正常">同步正常</option><option>接口资料不完整</option><option>同步失败</option></select></label><span class="data-filter-count">共 {{ activeRows.length }} 条</span></div>
      ...
      <aside class="data-record-drawer foundation-drawer" role="dialog" aria-modal="true" aria-label="基础数据详情">
        ...
        <div class="drawer-body">
          <template v-if="selectedDataset && detailTab === 'overview'">
            ...
            <section v-if="selectedDataset.status === '同步失败' && selectedFailureEvidence" class="detail-section"><h3>失败记录</h3><dl class="reference-facts"><div><dt>失败批次</dt><dd>{{ selectedFailureEvidence.batch }}</dd></div><div><dt>失败字段</dt><dd>{{ selectedFailureEvidence.field }}</dd></div><div><dt>校验规则</dt><dd>{{ selectedFailureEvidence.rule }}</dd></div><div><dt>已删除或遮盖敏感字段的样例</dt><dd>{{ selectedFailureEvidence.sample }}</dd></div><div><dt>来源责任方</dt><dd>{{ selectedFailureEvidence.owner }}</dd></div><div><dt>重试范围</dt><dd>{{ selectedFailureEvidence.retryScope }}</dd></div><div><dt>执行次数</dt><dd>{{ selectedFailureEvidence.attempts }}</dd></div><div><dt>错误代码</dt><dd>{{ selectedFailureEvidence.errorCode }}</dd></div></dl></section>
            ...
            <section v-if="selectedDataset.status === '接口资料不完整'" class="detail-section"><h3>阻塞原因</h3><div class="boundary-list"><p><AlertTriangle :size="16" />{{ selectedDataset.failureReason }}</p><p><ShieldCheck :size="16" />正式接入条件完成前不执行同步，也不向HIS开放读取。</p></div></section>
          </template>
          <template v-else-if="selectedDataset && detailTab === 'source'"><section class="detail-section"><h3>来源规则</h3><dl class="reference-facts"><div><dt>负责维护该数据的系统</dt><dd>{{ selectedDataset.source }}</dd></div><div><dt>稳定主键</dt><dd>{{ selectedDataset.stableKey }}</dd></div><div><dt>版本或更新时间</dt><dd>{{ selectedDataset.versionField }}</dd></div><div><dt>停用或删除</dt><dd>{{ selectedDataset.stopRule }}</dd></div><div><dt>历史可见性</dt><dd>{{ selectedDataset.historyVisibility }}</dd></div></dl></section><section class="detail-section"><h3>平台保存字段</h3><div class="boundary-list"><p><CheckCircle2 :size="16" />{{ selectedDataset.retainedFields }}</p><p><ShieldCheck :size="16" />只保存来源规则批准的基础字段，不从HIS消费需求反推来源字段。</p></div></section></template>
