...
const systems = ref<ExternalSystem[]>([
  { id: 'SYS-001', code: 'COUNTY_HIS', name: '县医院 HIS', description: '县医院业务数据提供系统', enabled: true, updatedAt: '2026-09-15 10:20', endpoints: [
  ...
  { id: 'SYS-002', code: 'PRIMARY_CARE', name: '基层业务系统', description: '基层卫生院业务协同系统', enabled: true, updatedAt: '2026-09-14 16:05', endpoints: [
...
function open(item: ExternalSystem, event?: MouseEvent) { drawerTrigger = event?.currentTarget instanceof HTMLElement ? event.currentTarget : document.activeElement instanceof HTMLElement ? document.activeElement : null; selected.value = item; draft.value = clone(item); isNew.value = false; drawerTab.value = 'endpoints'; void nextTick(() => drawerElement.value?.querySelector<HTMLElement>('button')?.focus()) }
function addSystem(event?: MouseEvent) {
...
function save() {
  ...
  if (invalidEndpoint) return emit('notify', endpointReadinessError(invalidEndpoint) ?? '服务地址配置未通过启用检查')
  ...
  emit('audit', { action: isNew.value ? '新增外部系统' : '更新外部系统', object: item.code, detail: `保存系统和 ${item.endpoints.length} 个环境服务地址；认证信息不显示明文` })
  emit('notify', '外部系统配置已保存'); close()
}
</script>

<template>
  <div class="external-panel">
    <section class="external-boundary"><ShieldCheck :size="21" /><div><strong>密码和密钥不在此页面填写</strong><small>如接口需要密码或密钥，只填写医院统一保管系统中的记录编号；启用前还需检查协议、超时和机构范围。</small></div><span class="badge bg-blue-lt text-blue">仅限授权人员</span></section>
    <div class="admin-summary"><ServerCog :size="20" /><div><strong>{{ systems.length }} 个外部系统</strong><small>{{ systems.reduce((sum, item) => sum + item.endpoints.filter(endpoint => endpoint.enabled).length, 0) }} 个服务地址当前启用</small></div><button class="prototype-button" @click="addSystem"><Plus :size="15" />新增外部系统</button></div>
    ...
    <section class="prototype-section work-table-section action-column-table external-system-table"><div class="prototype-table-wrap"><table class="work-table"><thead><tr><th>系统编码 / 名称</th><th>用途说明</th><th>服务地址</th><th>状态 / 最后修改</th><th>操作</th></tr></thead><tbody><tr v-for="item in pagedRows" :key="item.id"><td><button class="work-row-link" @click="open(item)">{{ item.name }}</button><small>{{ item.code }}</small></td><td><span class="cell-ellipsis" :title="item.description">{{ item.description }}</span></td><td><strong>{{ item.endpoints.filter(endpoint => endpoint.enabled).length }} / {{ item.endpoints.length }}</strong><small>启用 / 全部</small></td><td><span class="prototype-tag" :class="item.enabled ? 'success' : 'danger'">{{ item.enabled ? '已启用' : '已停用' }}</span><small>{{ item.updatedAt }}</small></td><td><button class="btn btn-outline-primary btn-sm" @click="open(item)">配置服务地址</button></td></tr></tbody></table><div v-if="!rows.length" class="prototype-empty">没有符合条件的外部系统</div></div><AdminPagination :total="rows.length" :page="page" :page-size="pageSize" @update:page="page = $event" @update:page-size="pageSize = $event" /></section>

    <div v-if="draft" ref="drawerElement" tabindex="-1" @keydown="onDrawerKeydown">
    <div v-if="draft" class="data-record-overlay" @click.self="close"><aside class="data-record-drawer external-drawer" role="dialog" aria-modal="true" aria-label="外部系统配置"><header class="drawer-titlebar"><div><small>{{ isNew ? '新增' : '编辑' }}</small><h2>外部系统配置</h2></div><button class="prototype-icon" aria-label="关闭" @click="close"><X :size="18" /></button></header><div class="admin-drawer-summary"><span class="prototype-tag" :class="draft.enabled ? 'success' : 'danger'">{{ draft.enabled ? '已启用' : '已停用' }}</span><strong>{{ draft.name || '待填写' }}</strong><small>{{ draft.code || '保存后生成标识' }}</small></div>
      <nav class="drawer-tabs"><button :class="{ active: drawerTab === 'detail' }" @click="drawerTab = 'detail'">基本信息</button><button :class="{ active: drawerTab === 'endpoints' }" @click="drawerTab = 'endpoints'">环境服务地址</button><button :class="{ active: drawerTab === 'checks' }" @click="drawerTab = 'checks'">启用检查</button></nav>
      <div class="admin-drawer-body"><div v-if="drawerTab === 'detail'" class="work-form work-form-compact"><label>系统编码</label><input v-model="draft.code" :disabled="!isNew" /><label>系统名称</label><input v-model="draft.name" /><label>用途说明</label><textarea v-model="draft.description" rows="3"></textarea><label>启用状态</label><select v-model="draft.enabled"><option :value="true">启用</option><option :value="false">停用</option></select></div>
        <template v-else-if="drawerTab === 'endpoints'"><div class="section-heading"><div><h3>环境服务地址</h3><small>同一系统、环境与机构范围只允许一个启用服务地址。</small></div><button class="btn btn-outline-primary btn-sm" @click="addEndpoint"><Plus :size="14" />新增服务地址</button></div><div class="endpoint-list"><article v-for="endpoint in draft.endpoints" :key="endpoint.id" class="endpoint-card"><header><strong>{{ endpoint.environment }} · {{ endpoint.organization }}</strong><span class="prototype-tag" :class="endpointReady(endpoint) ? 'success' : 'warning'">{{ endpointReady(endpoint) ? '检查通过' : '待完善' }}</span></header><div class="endpoint-grid"><label>环境<select v-model="endpoint.environment"><option>开发</option><option>测试</option><option>生产</option></select></label><label>机构范围<input v-model="endpoint.organization" /></label><label class="wide">基础地址<input v-model="endpoint.baseUrl" placeholder="http:// 或 https://" /></label><label>连接超时（毫秒）<input v-model.number="endpoint.connectTimeout" type="number" min="100" /></label><label>读取超时（毫秒）<input v-model.number="endpoint.readTimeout" type="number" min="100" /></label><label class="wide">密钥保管编号<KeyRound :size="14" /><input v-model="endpoint.credentialRef" placeholder="例如：县医院-HIS-生产（不填写密码）" /></label><label class="endpoint-check"><input v-model="endpoint.tls" type="checkbox" />使用 HTTPS/TLS</label><label class="endpoint-check"><input v-model="endpoint.enabled" type="checkbox" />启用服务地址</label></div></article><div v-if="!draft.endpoints.length" class="admin-empty">暂未配置环境服务地址</div></div></template>
        <div v-else class="check-list"><div><CheckCircle2 :size="18" /><span><strong>地址与协议</strong><small>允许正式接口实际使用的 HTTP 或 HTTPS 地址，TLS 标志必须与地址协议一致。</small></span></div><div><CheckCircle2 :size="18" /><span><strong>密码与密钥</strong><small>只保存医院统一保管系统中的记录编号，页面不显示密码或密钥明文。</small></span></div><div><CheckCircle2 :size="18" /><span><strong>超时与范围</strong><small>读取超时不得小于连接超时，机构范围由服务端再次校验。</small></span></div></div>
