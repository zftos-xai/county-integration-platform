...
function save() {
  ...
  emit('audit', { action: isNew.value ? '新增外部系统' : '更新外部系统', object: item.code, detail: `保存系统和 ${item.endpoints.length} 个环境服务地址（原型演示；凭证仅显示引用）` })
...
<template>
  <div class="external-panel">
    <section class="external-boundary"><ShieldCheck :size="21" /><div><strong>凭证不进入平台数据库</strong><small>页面只维护医院批准密钥来源的引用标识；启用服务地址前检查实际传输协议、超时和机构范围。</small></div><span class="badge bg-blue-lt text-blue">configuration:write</span></section>
    <div class="admin-summary"><ServerCog :size="20" /><div><strong>{{ systems.length }} 个外部系统</strong><small>{{ systems.reduce((sum, item) => sum + item.endpoints.filter(endpoint => endpoint.enabled).length, 0) }} 个服务地址当前启用 · 数据为合成演示</small></div><button class="prototype-button" @click="addSystem"><Plus :size="15" />新增外部系统</button></div>
    ...
    <section class="prototype-section work-table-section action-column-table"><div class="prototype-table-wrap"><table class="work-table"><thead><tr><th>系统编码 / 名称</th><th>用途说明</th><th>环境服务地址</th><th>启用服务地址</th><th>状态</th><th>最后修改</th><th>操作</th></tr></thead><tbody><tr v-for="item in pagedRows" :key="item.id"><td><button class="work-row-link" @click="open(item)">{{ item.name }}</button><small>{{ item.code }}</small></td><td>{{ item.description }}</td><td>{{ item.endpoints.length }} 个</td><td>{{ item.endpoints.filter(endpoint => endpoint.enabled).length }} 个</td><td><span class="prototype-tag" :class="item.enabled ? 'success' : 'danger'">{{ item.enabled ? '已启用' : '已停用' }}</span></td><td>{{ item.updatedAt }}</td><td><button class="btn btn-outline-primary btn-sm" @click="open(item)">配置服务地址</button></td></tr></tbody></table><div v-if="!rows.length" class="prototype-empty">没有符合条件的外部系统</div></div><AdminPagination :total="rows.length" :page="page" :page-size="pageSize" @update:page="page = $event" @update:page-size="pageSize = $event" /></section>
