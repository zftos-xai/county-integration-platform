  ...
  <div ref="listRoot" class="recovery-page recovery-focused" :class="{ 'has-detail': task && scenario }">
    ...
    <div v-if="task && scenario" class="recovery-detail-pane" @keydown.esc="closeDetail">
      <section :key="task.id" class="data-record-drawer recovery-detail-workspace" role="region" aria-labelledby="record-title">
        ...
        <div class="drawer-body">
          <template v-if="detailTab === 'overview'">
            ...
            <template v-if="task.kind === 'unknown'">
              <section class="detail-section reference-section"><div class="section-heading"><h3>核查依据</h3><span class="section-help"><IconInfoCircle :size="15" /> 信息来源</span></div><div class="evidence-card"><div><IconFileText :size="17" /><span>已发送请求</span><strong>{{ scenario.occurredAt }}</strong><button class="btn btn-link btn-sm" @click="emit('exchange', scenario.id)">查看请求报文</button></div><div><IconServer :size="17" /><span>接收系统响应</span><strong>{{ task.target.includes('无法') || task.target.includes('未知') ? '未收到明确响应' : task.target }}</strong></div><div><IconFile :size="17" /><span>本地数据文件</span><strong>{{ task.dataStatus === '暂存可用' ? `request_${scenario.requestId.slice(-4)}.xml（临时数据）` : '未保存业务正文' }}</strong></div><div><IconListDetails :size="17" /><span>相关日志</span><strong>应用日志 {{ scenario.steps.length + 4 }} 条</strong><button class="btn btn-link btn-sm" @click="emit('audit', scenario.requestId)">查看日志</button></div></div></section>
        ...
        <footer v-if="task.kind !== 'unknown'" class="drawer-footer"><span>当前为演示数据，不会触发真实发送或清理</span><button class="btn btn-outline-secondary btn-sm" @click="emit('rehearse', task.id)">重置演示状态</button></footer>
      </section>
    </div>
  </div>
</template>
