...
-- 主版本: V0400
-- 脚本类型: MAIN
-- 变更说明: 建立代码注册平台参数值、平台系统字典、外部系统和环境端点基线
-- 需求依据: 开发批次任务计划DEV-007、平台管理底座技术设计
-- 数据边界: 只保存获批参数键的作用域值和平台自身字典，不保存医疗业务目录、明文凭证或外部接口报文
-- 时间口径: created_at和updated_at保存UTC时间，精度为毫秒
...
-- 表中文名称: 平台参数值表
-- 表用途: 保存代码注册参数在环境及可选机构作用域下的值；不提供任意参数键创建能力，不保存外部系统明文凭证
CREATE TABLE sys_parameter_value (
    id BIGINT IDENTITY(1,1) NOT NULL CONSTRAINT pk_sys_parameter_value PRIMARY KEY, -- 参数值主键：平台参数作用域值内部主键
    ...
    is_enabled BIT NOT NULL CONSTRAINT df_sys_parameter_value_enabled DEFAULT 1, -- 启用标志：该作用域参数值是否参与平台配置解析
    ...
    CONSTRAINT uq_sys_parameter_value_scope UNIQUE (parameter_key, environment_code, organization_id), -- 保证同一参数键在同一环境和机构作用域只有一个值
    CONSTRAINT fk_sys_parameter_value_organization FOREIGN KEY (organization_id) REFERENCES org_organization(id), -- 保证可选机构作用域引用平台机构
...
CREATE TABLE sys_external_system (
    ...
    is_enabled BIT NOT NULL CONSTRAINT df_sys_external_system_enabled DEFAULT 1, -- 启用标志：是否允许其端点参与解析
...
-- 表中文名称: 外部系统端点表
-- 表用途: 保存外部系统在环境和可选机构作用域下的受控连接参数；凭证字段仅保存批准密钥来源的引用，不保存明文凭证
CREATE TABLE sys_external_endpoint (
    id BIGINT IDENTITY(1,1) NOT NULL CONSTRAINT pk_sys_external_endpoint PRIMARY KEY, -- 端点主键：平台内部主键
    external_system_id BIGINT NOT NULL, -- 外部系统主键：所属已确认系统
    environment_code NVARCHAR(16) NOT NULL, -- 环境代码：DEVELOPMENT、TEST或PRODUCTION
    organization_id BIGINT NULL, -- 机构主键：机构级端点；全局端点为空
    ...
    is_enabled BIT NOT NULL CONSTRAINT df_sys_external_endpoint_enabled DEFAULT 0, -- 启用标志：新端点默认停用，核对后显式启用
    created_by NVARCHAR(64) NOT NULL, -- 创建人标识：平台登录名快照
    ...
    row_version ROWVERSION, -- 并发版本：防止覆盖其他管理员修改
    CONSTRAINT uq_sys_external_endpoint_scope UNIQUE (external_system_id, environment_code, organization_id), -- 保证系统、环境和机构作用域唯一
    CONSTRAINT fk_sys_external_endpoint_system FOREIGN KEY (external_system_id) REFERENCES sys_external_system(id), -- 保证端点所属系统存在
    CONSTRAINT fk_sys_external_endpoint_organization FOREIGN KEY (organization_id) REFERENCES org_organization(id), -- 保证机构作用域存在
...
VALUES
    ...
    (N'CONSTRAINT', N'sys_parameter_value', N'pk_sys_parameter_value', N'保证每个参数作用域值具有唯一内部主键。'),
    (N'CONSTRAINT', N'sys_parameter_value', N'uq_sys_parameter_value_scope', N'保证同一参数键在同一环境和机构作用域只有一个值。'),
    (N'CONSTRAINT', N'sys_parameter_value', N'fk_sys_parameter_value_organization', N'保证可选机构作用域引用平台机构。'),
    ...
    (N'CONSTRAINT', N'sys_parameter_value', N'df_sys_parameter_value_enabled', N'新参数作用域值默认启用。'),
    ...
    (N'TABLE', N'sys_external_endpoint', NULL, N'外部系统端点表'),
    (N'COLUMN', N'sys_external_endpoint', N'id', N'端点主键'),
    ...
    (N'CONSTRAINT', N'sys_external_endpoint', N'pk_sys_external_endpoint', N'保证每个外部系统端点具有唯一内部主键。'),
    (N'CONSTRAINT', N'sys_external_endpoint', N'uq_sys_external_endpoint_scope', N'保证系统、环境和机构作用域唯一。'),
    (N'CONSTRAINT', N'sys_external_endpoint', N'fk_sys_external_endpoint_system', N'保证端点所属外部系统存在。'),
    (N'CONSTRAINT', N'sys_external_endpoint', N'fk_sys_external_endpoint_organization', N'保证可选机构作用域存在。'),
    (N'CONSTRAINT', N'sys_external_endpoint', N'ck_sys_external_endpoint_environment', N'限制为平台支持的部署环境。'),
    (N'CONSTRAINT', N'sys_external_endpoint', N'ck_sys_external_endpoint_url_not_blank', N'禁止保存空白端点地址。'),
    ...
    (N'CONSTRAINT', N'sys_external_endpoint', N'df_sys_external_endpoint_enabled', N'新外部系统端点默认停用。'),
    ...
    (N'INDEX', N'sys_external_endpoint', N'ix_sys_external_endpoint_organization_environment', N'支持按机构范围、环境和启用状态解析端点。');
