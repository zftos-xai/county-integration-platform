...
-- 主版本: V0500
-- 脚本类型: MAIN
-- 变更说明: 建立不可由普通管理功能修改或删除的管理审计记录基线
-- 需求依据: 开发批次任务计划DEV-009、平台管理底座技术设计
-- 数据边界: 保存操作人和机构快照、目标、结果及脱敏摘要，不保存密码、Token、验证码、凭证引用、私钥或业务正文
-- 时间口径: occurred_at保存UTC时间，精度为毫秒
-- 回退方案: 首次部署失败时删除audit_management_event；共享环境执行后只允许新增归属V0500的版本化补丁

-- 表中文名称: 管理审计事件表
-- 表用途: 追加保存平台管理变更与安全失败事件；普通业务服务不提供更新或删除能力
CREATE TABLE audit_management_event (
    ...
    actor_user_id BIGINT NULL, -- 操作人主键：认证成功时保存；登录失败等无可信主体时为空
    actor_login_snapshot NVARCHAR(128) NOT NULL, -- 操作人快照：认证主体登录名或脱敏登录提示
    ...
    CONSTRAINT ck_audit_management_event_actor_not_blank CHECK (LEN(LTRIM(RTRIM(actor_login_snapshot))) > 0), -- 禁止空白操作人快照
...
INSERT INTO @object_descriptions (object_type, table_name, object_name, description) VALUES
    ...
    (N'COLUMN', N'audit_management_event', N'actor_login_snapshot', N'操作人快照'),
    ...
    (N'CONSTRAINT', N'audit_management_event', N'ck_audit_management_event_actor_not_blank', N'禁止保存空白操作人快照。'),
    ...
    (N'CONSTRAINT', N'audit_management_event', N'ck_audit_management_event_summary_not_blank', N'禁止保存空白脱敏摘要。'),
