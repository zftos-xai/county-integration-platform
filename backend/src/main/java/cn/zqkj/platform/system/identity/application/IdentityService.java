...
public class IdentityService {
    ...
    public BootstrapStatus getBootstrapStatus() {
        boolean initialized = repository.countUsers() > 0;
        return new BootstrapStatus(initialized, !initialized && isBootstrapSecretConfigured());
    }
     ...
     */
    @Transactional
    public BootstrapResult bootstrap(String suppliedSecret, BootstrapCommand command) {
        validateBootstrapSecret(suppliedSecret);
        BootstrapCommand normalized = normalize(command);
        if (repository.lockAndCountUsers() > 0) {
            throw new ResourceConflictException("Platform bootstrap has already completed");
        }

        OrganizationView organization = organizationService.create(
        ...
        INITIAL_PERMISSIONS.forEach(repository::upsertPermission);
        long roleId = repository.createRole("PLATFORM_ADMIN", "平台管理员", BOOTSTRAP_ACTOR);
        ...
        INITIAL_PERMISSIONS.keySet().forEach(code -> repository.grantPermission(roleId, code, BOOTSTRAP_ACTOR));
        repository.grantRole(userId, roleId, BOOTSTRAP_ACTOR);
        repository.grantOrganization(userId, organization.id(), BOOTSTRAP_ACTOR);
    ...
    public void changePassword(long userId, String currentPassword, String newPassword) {
        ...
        if (repository.changePassword(userId, passwordEncoder.encode(newPassword), account.loginName()) != 1) {
