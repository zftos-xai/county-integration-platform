...
/**
 * 提供数据库契约扫描、方案、审批、执行和取消API。
 *
 *
 * <p>方案属于平台级维护资源；功能权限在入口校验，审批与并发规则由服务层落实。</p>
 */
@Validated
@RestController
...
public class DatabaseContractMaintenanceController {
     ...
     *
     * <p>需要 {@code database-contract:read} 功能权限。</p>
     *
     * @return 当前实时契约差异
     */
     ...
     * 根据当前仍存在的契约差异创建维护方案。
     *
     * <p>需要 {@code database-contract:plan} 功能权限。</p>
     *
     * @param request 创建请求
    ...
    public ApiResponse<DatabaseContractPlanVO> createPlan(
        ...
        return ApiResponse.success(service.createPlan(request, principal.accessActor()));
    }

    /**
     * 审批尚未执行的数据库契约维护方案。
     *
     * <p>需要 {@code database-contract:approve} 功能权限。</p>
     *
     * @param id 主键
     * @param request 审批请求
     * @param principal 当前用户
    ...
    public ApiResponse<DatabaseContractPlanVO> approve(
        ...
        return ApiResponse.success(service.approve(
                id, Func.decodeRowVersion(request.version()), request.note(), principal.accessActor()));
    }

    /**
     * 执行已批准方案并在同一受控流程中完成复验。
     *
     * <p>需要 {@code database-contract:execute} 功能权限。</p>
     *
     * @param id 主键
     * @param request 并发版本
     * @param principal 当前用户
    ...
    public ApiResponse<DatabaseContractPlanVO> execute(
            ...
            @AuthenticationPrincipal PlatformUserPrincipal principal) {
        return ApiResponse.success(service.execute(id, Func.decodeRowVersion(request.version()), principal.accessActor()));
    }
    ...
    public ApiResponse<DatabaseContractPlanVO> cancel(
            ...
            @Valid @RequestBody AdvanceDatabaseContractPlanRequest request,
            @AuthenticationPrincipal PlatformUserPrincipal principal) {
        return ApiResponse.success(service.cancel(id, Func.decodeRowVersion(request.version()), principal.accessActor()));
    }
