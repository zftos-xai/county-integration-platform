...
class AccessMapperXmlTest {
    ...
    void parsesAccessMapper() throws Exception {
        ...
        String resource = "mapper/system/identity/AccessMapper.xml";
        try (InputStream input = Resources.getResourceAsStream(resource)) {
            new XMLMapperBuilder(input, configuration, resource, configuration.getSqlFragments()).parse();
        }

        assertTrue(configuration.hasStatement(
                "cn.zqkj.platform.system.identity.mapper.AccessMapper.replaceUserRoles"
        ));
        assertTrue(configuration.hasStatement(
                "cn.zqkj.platform.system.identity.mapper.AccessMapper.countPermissions"
        ));
    }
}
