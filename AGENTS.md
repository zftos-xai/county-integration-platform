...
- 工程规则分为团队底线、技术栈规范和项目覆盖项，制定、例外、版本与跨项目推广遵守 `docs/standards/团队工程规范管理办法.md`。
- 本地提交前运行 `./tools/verify.sh`；CI 必须执行同等检查，不得只以人工确认代替可自动化强制检查。
- 跨项目复用使用 `plugins/team-engineering-standards`，但插件建议不得覆盖目标仓库的 `AGENTS.md` 或项目专项规范。
- 规则例外必须记录影响范围、风险、失败后补救措施、负责人和到期日；敏感数据泄露、虚假验证证据和未经授权的破坏性操作不得例外放行。
...
- 已进入共享环境后的补丁必须引用归属主版本、连续编号并通过`node tools/verify-sql-migrations.mjs`；表、字段、约束、索引和时间规则必须有准确注释。
...
- 所有导出的 TypeScript 类型、类、常量和函数必须使用准确的 TSDoc 说明职责或使用约定；Vue 单文件组件必须在文件顶部说明页面或组件职责。权限、会话、并发控制、敏感数据和兼容性等非显然逻辑必须解释“为什么”，不得用逐行翻译代码的无效注释充数。
- 正式页面、嵌入页面和合成原型的认证及业务状态必须隔离；前端权限、页面访问权限检查和表单校验不得替代服务端授权、机构范围及数据校验。
- TypeScript保持严格模式，禁止使用`any`、`@ts-ignore`或不安全断言掩盖接口定义问题；API变化必须同步类型、调用方、错误处理和测试。
