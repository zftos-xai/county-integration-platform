...
- 工程规则分为团队底线、技术栈规范和项目覆盖项，制定、例外、版本与跨项目推广遵守 `docs/standards/team-engineering-governance.md`。
- 本地提交前运行 `./tools/verify.sh`；CI 必须执行同等检查，不得只以人工确认代替可自动化门禁。
- 跨项目复用使用 `plugins/team-engineering-standards`，但插件建议不得覆盖目标仓库的 `AGENTS.md` 或项目专项规范。
- 规则例外必须记录影响范围、风险、补偿措施、负责人和到期日；敏感数据泄露、虚假验证证据和未经授权的破坏性操作不得例外放行。
